    ...
    const columns = useMemo(
        () => [
            columnHelper.accessor("imagenes_url", {
                header: "IMAGEN",
                cell: (info) => {
                    const src = info.getValue()?.length > 0 ? info.getValue()[0] : null;
                    return src ? (
                        <img
                            src={src}
                        ...
                        <div className="w-12 h-12 rounded-xl bg-amber-100 text-amber-700 font-semibold flex items-center justify-center text-xs">
